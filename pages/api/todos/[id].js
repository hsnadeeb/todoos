  ...
  try {
    ...
    switch (method) {
      ...
      case 'PUT':
        ...
        case 'PATCH': // Use PATCH for updating a specific field
  const { status } = req.body;
  const updatedStatusTodo = await collection.findOneAndUpdate(
    { _id: new ObjectId(id) },
    { $set: { status } },
    { returnDocument: 'after' }
  );
  if (!updatedStatusTodo.value) {
    return res.status(404).json({ success: false, error: 'Todo not found' });
  }
  res.status(200).json({ success: true, data: updatedStatusTodo.value });
  break;
